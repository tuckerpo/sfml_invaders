...
void UFO::update(float dt) {
    auto getX = [](sf::RectangleShape& sprite) { return sprite.getPosition().x; };
    switch (m_ufo_state) {
        case UFOState::None:
            getInitialDir();
            break;
...
void UFO::input(Keyboard& kb) {
    if (kb.is_key_down(sf::Keyboard::Key::Pause)) m_ufo_state = UFOState::Paused;
    return;
}

const EntityType UFO::getType() const { return m_Type; }
...
const bool UFO::isAlive() const { return m_ufo_state != UFOState::Destroyed; }