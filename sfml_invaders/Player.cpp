...
const EntityType Player::getType() const { return m_Type; }

void Player::update(float dt) {
    float width = m_sprite.getGlobalBounds().width;

    // NOTE(tjp): projectiles belonging to a player should continue regardless of current frame's
    // player state.
    ...
    switch (m_PlayerState) {
        ...
        case PlayerState::Alive:
            ...
            // m_sprite.getPosition().y, m_velocity.x, m_velocity.y);
...
void Player::input(Keyboard& kb) {
    ...
    if (kb.is_key_down(sf::Keyboard::Key::LShift)) speed *= 2;
...
void Player::onCollide(Collidable& other) { m_PlayerState = PlayerState::Dead; }

const uint16_t Player::getNumLives() const { return m_numLives; }

const uint16_t Player::getScore() const { return m_score; }

const bool Player::isAlive() const { return m_numLives > 0; }
